...
def create_image_menu(state, menu_bar):
    def update_display_image():
        state["cv_image_display"] = state["cv_image_full"]
        state["tk_image"] = helper.cv2_to_tk(state["cv_image_display"])
        state["canvas"].delete("all")
        state["canvas"].create_image(0, 0, anchor="nw", image=state["tk_image"])
    ...
    def start_rectangle(event):
        reset_selection()
        state["selection_points"].append(clamp_to_image(event.x, event.y))
    ...
    def update_rectangle(event):
        state["selection_points"][1] = clamp_to_image(event.x, event.y)
    ...
    def finish_rectangle(event):
        ...
        state["selection_points"] = scale_up_cords(state["selection_points"])
        x1, y1 = state["selection_points"][0]
        x2, y2 = state["selection_points"][1]
    ...
    def finish_lasso(event):
        ...
        if len(state["selection_points"]) > 2:
            line_id = state["canvas"].create_line(
                state["selection_points"][-1],
                state["selection_points"][0],
                fill="red", width=2
            )
            state["selection_shape_ids"].append(line_id)

        state["selection_points"] = scale_up_cords(state["selection_points"])
        pts = np.array(state["selection_points"], np.int32)
        mask = np.zeros(state["cv_image_full"].shape[:2], dtype=np.uint8)
        cv2.fillPoly(mask, [pts], 255)
    ...
    menu_select.add_command(label="Crop", command=lambda: state["canvas"].bind("<Button-1>", start_crop))
